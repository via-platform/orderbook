    ...
    constructor({manager, omnibar}, state = {}){
        ...
        this.columns = [
            ...
            },
            {
                ...
                align: 'right',
                element: row => {
                    const precision = (typeof this.market.precision.amount === 'number') ? this.market.precision.amount : 8;

                    if(precision <= 0){
                        return $.div({classList: 'td size'}, $.span({}, row.size));
                    }

                    let head = row.size.toFixed(precision).replace(/0+$/g, '');

                    if(head.slice(-1) === '.'){
                        head += '0';
                    }

                    const [numerals, decimals] = head.split('.');
                    const tail = '00000000';

                    return $.div({classList: 'td size'}, $.span({}, head), tail.slice(decimals.length, precision));
                }
            },
            {
                name: 'price',
